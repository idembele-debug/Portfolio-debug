...
export default function HistoireOverlay({ isOpen, onClose }) {
  ...
  return (
    <div id="histoire" className="fixed inset-0 z-50 bg-[var(--bg)] overflow-y-auto animate-fadeIn">
      ...
      <div className="max-w-[820px] mx-auto px-5 sm:px-8 md:px-12 py-20 md:py-28 lg:py-32">
        ...
        <h1 className="font-sans text-3xl sm:text-4xl font-extrabold tracking-[-0.03em] mb-3">{t('histoire.title')}</h1>
        <p className="text-[13px] sm:text-sm text-[var(--muted)] mb-12 sm:mb-14 leading-relaxed">{t('histoire.subtitle')}</p>
        ...
        {loading ? (
          ...
          <div className="space-y-10 sm:space-y-12">
            {chapters.map((chapter) => (
              <article key={chapter.id} className="relative pl-8 sm:pl-10 border-l-2 border-[var(--border)]">
                ...
                <h2 className="font-sans text-lg sm:text-xl font-bold text-[var(--accent)] mb-4 tracking-[-0.01em]">
                  {chapter.title}
                </h2>
                <div className="text-[13px] sm:text-sm text-[var(--muted)] leading-[1.85] space-y-3">
