...
export default function ProjectsSection() {
  ...
  return (
    <section id="projects" className="max-w-[1200px] mx-auto px-6 md:px-12 py-20">
      <div className="text-[11px] text-[var(--muted)] tracking-[0.1em] uppercase mb-11 flex items-center gap-3">
        ...
        <span className="text-[var(--accent)] mr-1">#</span> projects
      </div>

      <div className="flex items-center gap-2 mb-9 flex-wrap">
        ...
        <span className="ml-auto text-[11px] text-[var(--muted)]">{filtered.length} projects</span>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border border-[var(--border)] rounded-xl overflow-hidden">
        {filtered.map((project) => (
          <div
            key={project.id}
            className="bg-[var(--bg2)] p-6 transition-colors cursor-pointer relative overflow-hidden hover:bg-[var(--bg3)] group"
          >
            ...
            <div className={`w-full h-[130px] rounded mb-4 border border-[var(--border)] flex items-center overflow-hidden bg-gradient-135 ${PREVIEW_COLORS[project.preview_color || 'blue'] || 'from-[#0f172a] to-[#1e3a5f]'}`}>
              <pre className="font-mono text-[10px] text-white/35 p-3 leading-[1.7] w-full whitespace-pre-wrap">
                {project.preview_code || '// No preview available'}
              </pre>
            </div>

            <div className="flex items-start justify-between mb-1.5">
              <span className="font-sans font-bold text-sm text-[var(--text)]">{project.title}</span>
              <span className={`text-[9px] px-1.5 py-0.5 rounded border uppercase tracking-[0.05em] ${TYPE_STYLES[project.project_type] || 'text-[var(--muted)] border-[var(--border)]'}`}>
