...
export default function ContactOverlay({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[var(--bg)] overflow-y-auto animate-fadeIn">
      ...
      <div className="max-w-[1100px] mx-auto px-5 sm:px-8 md:px-12 lg:px-16 py-20 md:py-28 lg:py-32">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-12 lg:gap-16 xl:gap-24 items-start">
          {/* Left column */}
          <div className="space-y-10 sm:space-y-12">
            <p className="text-[14px] sm:text-[15px] text-[var(--text)] italic leading-relaxed max-w-md">
              {t('contact.intro')}
            </p>
            ...
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-8">
                ...
                <span className="text-[14px] text-[var(--text)]">{phone}</span>
              </div>
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-8">
                <span className="text-[11px] text-[var(--muted)] uppercase tracking-wider w-20 shrink-0">location</span>
                <span className="text-[14px] text-[var(--text)]">{location}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-8 sm:space-y-10">
            <h2 className="font-sans text-3xl sm:text-4xl lg:text-[2.5rem] font-extrabold tracking-[-0.02em] leading-tight">
              {t('contact.heading')}
            </h2>
            ...
            <div className="space-y-5">
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-8">
                ...
                <a href={`mailto:${email}`} className="text-[14px] text-[var(--accent)] no-underline hover:underline">
                  {email}
                </a>
              </div>
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-8">
                <span className="text-[11px] text-[var(--muted)] uppercase tracking-wider w-20 shrink-0">linkedin</span>
                <a href={linkedinUrl} target="_blank" rel="noopener noreferrer" className="text-[14px] text-[var(--accent)] no-underline hover:underline">
                  ISSA D DEMBELE
                </a>
              </div>
            </div>
            ...
            {sent ? (
              <div className="text-center py-12 animate-fadeIn">
                ...
                <p className="text-sm text-[var(--muted)] mb-6">{t('contact.form.sent.text')}</p>
                <button
                  type="button"
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 sm:space-y-6">
                <div className="flex flex-col gap-2">
                  ...
                  <input
                    ...
                    className="bg-[var(--bg2)] border border-[var(--border)] rounded-lg px-4 py-3 text-sm text-[var(--text)] font-mono outline-none focus:border-[var(--accent)] transition-colors w-full"
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <textarea
                    ...
                    rows={5}
                    className="bg-[var(--bg2)] border border-[var(--border)] rounded-lg px-4 py-3 text-sm text-[var(--text)] font-mono outline-none focus:border-[var(--accent)] transition-colors resize-none w-full"
                ...
                <button
                  type="submit"
                  className="w-full bg-[var(--accent)] border-none text-black font-mono text-sm font-bold py-3.5 rounded-lg cursor-pointer hover:opacity-90 transition-opacity"
                >
                  {t('contact.form.send')}
                </button>
              </form>
