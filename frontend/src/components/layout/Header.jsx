...
export default function Header({ onOpenAbout, onOpenContact, onOpenHistoire }) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 sm:px-6 md:px-12 h-14 bg-[var(--bg)]/92 backdrop-blur-md border-b border-[var(--border)]">
      ...
      {mobileOpen && (
        <div className="absolute top-14 left-0 right-0 bg-[var(--bg)]/98 backdrop-blur-md border-b border-[var(--border)] md:hidden animate-fadeIn">
          <div className="flex flex-col gap-1 p-4">
            ...
            <button onClick={() => { onOpenHistoire(); setMobileOpen(false); }} className="text-xs tracking-wider text-[var(--muted)] hover:text-[var(--text)] transition-colors cursor-pointer bg-transparent border-none font-mono text-left py-2 px-3 rounded hover:bg-[var(--bg2)]">
              {t('nav.histoire')}
            </button>
          </div>
        </div>
