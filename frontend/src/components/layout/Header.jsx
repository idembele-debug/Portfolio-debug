...
export default function Header({ onOpenAbout, onOpenContact }) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-5 sm:px-8 lg:px-12 xl:px-16 h-16 sm:h-[4.5rem] bg-[var(--bg)]/95 backdrop-blur-md border-b border-[var(--border)]">
      <a href="#" className="font-mono font-black text-sm sm:text-base md:text-lg tracking-widest uppercase no-underline text-[var(--text)] whitespace-nowrap">
        <span className="text-[var(--accent)]">Portfolio</span> ISSA D<span className="text-[var(--accent2)]">.</span>
      </a>
      ...
      {mobileOpen && (
        <div className="absolute top-16 left-0 right-0 bg-[var(--bg)]/98 backdrop-blur-md border-b border-[var(--border)] md:hidden animate-fadeIn">
          <div className="flex flex-col gap-1 p-4">
            <button onClick={() => { onOpenAbout(); setMobileOpen(false); }} className="text-xs tracking-wider text-[var(--muted)] hover:text-[var(--text)] transition-colors cursor-pointer bg-transparent border-none font-mono text-left py-2.5 px-3 rounded hover:bg-[var(--bg2)]">
              {t('nav.about')}
            </button>
            <a href="#projects" onClick={() => setMobileOpen(false)} className="text-xs tracking-wider text-[var(--muted)] hover:text-[var(--text)] transition-colors no-underline py-2.5 px-3 rounded hover:bg-[var(--bg2)]">
              {t('nav.projects')}
            </a>
            <button onClick={() => { onOpenContact(); setMobileOpen(false); }} className="text-xs tracking-wider text-[var(--muted)] hover:text-[var(--text)] transition-colors cursor-pointer bg-transparent border-none font-mono text-left py-2.5 px-3 rounded hover:bg-[var(--bg2)]">
              {t('nav.contact')}
            </button>
          </div>
        </div>
